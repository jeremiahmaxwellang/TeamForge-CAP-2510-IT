...
function displayApplicantDetails(applicant) {
    ...
    if (topBadge) {
        ...
        if (currentStatus === 'Accepted') {
            topBadge.style.backgroundColor = '#4CAF50';
            topBadge.style.color = 'white';
            const actionContainer = document.getElementById('actionContainer');
            if (actionContainer) actionContainer.style.display = 'block';
        } else if (currentStatus === 'Rejected') {
            topBadge.style.backgroundColor = '#f44336';
            topBadge.style.color = 'white';
        } else {
            topBadge.style.backgroundColor = '#fff3cd';
            topBadge.style.color = '#856404';
        }
    }

    // Build the GPA / CGPA display values.
    // lastGPA maps to GPA (term), CGPA maps to cumulative GPA.
    const gpaDisplay  = applicant.lastGPA != null
        ? parseFloat(applicant.lastGPA).toFixed(2)
        : '—';
    const cgpaDisplay = applicant.CGPA != null
        ? parseFloat(applicant.CGPA).toFixed(2)
        : '—';
    ...
    const details = [
        ...
        { field: 'GPA (Term)',          value: gpaDisplay },
        { field: 'CGPA (Cumulative)',   value: cgpaDisplay },
